...
async fn list_providers(State(state): State<AppState>) -> Json<Value> {
    ...
    let items: Vec<Value> = providers.iter().map(|p| serde_json::to_value(p).unwrap_or(Value::Null)).collect();
...
async fn get_provider(State(state): State<AppState>, Path(id): Path<String>) -> Json<Value> {
    ...
    let provider = providers.iter().find(|p| {
        serde_json::to_value(p).unwrap_or_default()
            .get("id").and_then(|v| v.as_str()) == Some(&id)
            || serde_json::to_value(p).unwrap_or_default()
            .get("name").and_then(|v| v.as_str()) == Some(&id)
    });
    Json(provider.map(|p| serde_json::to_value(p).unwrap_or_default()).unwrap_or(Value::Null))
}
...
async fn list_plugins(State(state): State<AppState>) -> Json<Value> {
    ...
    let items: Vec<Value> = registry.list().iter().map(|s| {
        json!({
            "name": s.name,
            "version": s.version,
...
async fn get_plugin(State(state): State<AppState>, Path(id): Path<String>) -> Json<Value> {
    let lock = state.plugin_manager.read().await;
    let registry = lock.registry();
    let plugin = registry.list().iter().find(|s| s.name == id);
...
async fn install_plugin(State(state): State<AppState>, Json(body): Json<Value>) -> Json<Value> {
    ...
    match registry.install(&identifier).await {
        Ok(_) => {
            broadcast_plugin_change(&state, "installed", &identifier).await;
            Json(json!({"success": true, "installed": identifier}))
        }
...
async fn uninstall_plugin(State(state): State<AppState>, Path(id): Path<String>) -> Json<Value> {
    let mut lock = state.plugin_manager.write().await;
    let registry = lock.registry_mut();
    match registry.uninstall(&id).await {
        Ok(_) => {
            broadcast_plugin_change(&state, "uninstalled", &id).await;
...
async fn toggle_plugin(State(state): State<AppState>, Path(id): Path<String>, Json(body): Json<Value>) -> Json<Value> {
    ...
    match result {
        Ok(_) => {
            broadcast_plugin_change(&state, if enable { "enabled" } else { "disabled" }, &id).await;
            Json(json!({"success": true, "id": id, "enabled": enable}))
        }
        Err(e) => Json(json!({"success": false, "error": e.to_string()})),
    }
}
...
async fn update_persona(State(state): State<AppState>, Path(id): Path<String>, Json(body): Json<CustomPersonaRequest>) -> Json<Value> {
    let mut mgr = state.persona_manager.lock().unwrap();
    match mgr.update_persona(&id, &body) {
        Ok(_) => Json(json!({"success": true, "updated": id})),
        Err(e) => Json(json!({"success": false, "error": e.to_string()})),
    }
}
    ...
    match &state.db {
        Some(db) => {
            match db.get_session(&id).await {
                ...
                Err(_) => Json(json!({"success": false, "error": "Session not found"})),
            }
        }
...
async fn delete_session(State(state): State<AppState>, Path(id): Path<String>) -> Json<Value> {
    match &state.db {
        Some(db) => match db.delete_session(&id).await {
            Ok(_) => Json(json!({"success": true, "deleted": id})),
            Err(e) => Json(json!({"success": false, "error": e.to_string()})),
        },
        None => Json(json!({"success": false, "error": "Database not configured"})),
...
async fn get_session_history(State(state): State<AppState>, Path(id): Path<String>, Query(params): Query<HashMap<String, String>>) -> Json<Value> {
    ...
    match &state.db {
        Some(db) => {
            match db.get_session_messages(&id, limit).await {
                Ok(messages) => {
...
async fn list_history(State(state): State<AppState>, Query(params): Query<HashMap<String, String>>) -> Json<Value> {
    ...
    match &state.db {
        Some(db) => {
            match db.list_messages(limit).await {
                Ok(messages) => {
                    let items: Vec<Value> = messages.iter().map(|m| {
                        json!({
                            "id": m.id,
                    ...
                    Json(json!({"messages": items, "count": items.len()}))
                }
                Err(e) => Json(json!({"success": false, "error": e.to_string()})),
            }
        }
...
async fn get_message(State(state): State<AppState>, Path(id): Path<String>) -> Json<Value> {
    match &state.db {
        Some(db) => {
            match db.get_message(&id).await {
                Ok(msg) => Json(json!({
                    "id": msg.id,
                    "role": msg.role,
                    "content": msg.content,
                ...
                Err(_) => Json(json!({"success": false, "error": "Message not found"})),
            }
        }
        None => Json(json!({"success": false, "error": "Database not configured"})),
    }
...
async fn delete_message(State(state): State<AppState>, Path(id): Path<String>) -> Json<Value> {
    match &state.db {
        Some(db) => match db.delete_message(&id).await {
            Ok(_) => Json(json!({"success": true, "deleted": id})),
            Err(e) => Json(json!({"success": false, "error": e.to_string()})),
        },
        None => Json(json!({"success": false, "error": "Database not configured"})),
...
async fn get_logs(State(state): State<AppState>) -> Json<Value> {
    match &state.log_broadcaster {
        Some(broadcaster) => {
            let logs = broadcaster.recent_logs().await;
            Json(json!({"logs": logs}))
        }
        None => Json(json!({"logs": []})),
    }
