use crate::api::{
    broadcast_config_update, broadcast_plugin_change, broadcast_provider_status,
...
use astrbot_provider::{
    ChatMessage as ProviderChatMessage, ChatOptions, ChatProvider, ProviderConfig,
};
use axum::{
...
fn build_router(state: AppState) -> Router {
    Router::new()
        ...
        .route(
            "/api/config",
            get(get_config).put(update_config_with_broadcast),
        )
        .route(
            "/api/config/:key",
...
async fn test_provider(State(state): State<AppState>, Path(id): Path<String>) -> Json<Value> {
    ...
    let test_msg =
        ProviderChatMessage::user("Hello, this is a connectivity test from AstrBot Dashboard.");
...
async fn _test_provider_chat(
    ...
    let provider: Box<dyn ChatProvider> = match provider_type {
        ...
        _ => {
            return json!({"success": false, "provider": id, "error": format!("Provider type '{}' not supported", provider_type), "message": "Provider test failed"})
        }
    };

    match provider.chat(vec![test_msg], options).await {
        Ok(reply) => {
            json!({"success": true, "provider": id, "reply_preview": reply.chars().take(100).collect::<String>(), "message": "Provider is online and responding"})
        }
        Err(e) => {
            json!({"success": false, "provider": id, "error": e.to_string(), "message": "Provider test failed"})
        }
    }
}
...
async fn create_persona(
    State(state): State<AppState>,
    Json(body): Json<CustomPersonaRequest>,
) -> Json<Value> {
    let mgr = state.persona_manager.lock().unwrap();
...
async fn list_sessions(State(state): State<AppState>) -> Json<Value> {
    match &state.db {
        Some(db) => match db.list_sessions(100).await {
            Ok(sessions) => {
...
async fn get_session(State(state): State<AppState>, Path(id): Path<String>) -> Json<Value> {
    match &state.db {
        Some(db) => match db.get_session(&id).await {
...
async fn get_session_history(
    ...
    let limit = params
        ...
        .unwrap_or(50);
    match &state.db {
        Some(db) => match db.get_session_messages(&id, limit as i64).await {
            Ok(messages) => {
...
async fn update_setting(
    ...
    match key.as_str() {
        "nickname" => {
            if let Some(v) = body.as_str() {
                cfg.nickname = v.to_string();
            }
        }
        "prefixes" => {
        ...
        match socket.recv().await {
            Some(Ok(WsMessage::Text(text))) => {
                let req: ChatRequest = match serde_json::from_str(&text) {
                    ...
                    Err(e) => {
                        let err =
                            json!({"type": "error", "message": format!("Invalid request: {}", e)});
                ...
                if req.stream {
                    ...
                    match provider.chat(messages, config).await {
                        ...
                        Err(e) => {
                            let err =
                                json!({"type": "error", "message": format!("Chat error: {}", e)});
                            let _ = socket.send(WsMessage::Text(err.to_string())).await;
                        }
                    }
                } else {
