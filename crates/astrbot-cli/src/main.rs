...
async fn cmd_run(
    ...
    info!("Starting AstrBot, press Ctrl+C to stop...");
    runtime.start().await?;
    info!("Shutting down...");
...
async fn cmd_config(show: bool, set: Option<String>, file: &str) -> anyhow::Result<()> {
    if show {
    ...
    if let Some(kv) = set {
        ...
        if parts.len() != 2 {
            println!("❌ Invalid format: expected KEY=VALUE, got {}", kv);
