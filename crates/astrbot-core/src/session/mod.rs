...
impl SessionManager {
    ...
    fn session_id(source: &MessageSource) -> String {
        format!("{:?}_{}_{}", source.platform, source.session_id, source.user_id)
    }
    ...
    pub async fn build_context(
        ...
        let mut messages = self.db.get_messages_as_chat(
            &session_id, limit
        ).await?;
...
mod tests {
    ...
    async fn test_save_user_and_assistant_messages() {
        ...
        manager.save_user_message(&source, "Hello bot").await.unwrap();
        manager.save_assistant_message(&source, "Hello user", Some("gpt-4")).await.unwrap();
    ...
    async fn test_build_context_with_system_prompt() {
        ...
        manager.save_user_message(&source, "What's 1+1?").await.unwrap();
        manager.save_assistant_message(&source, "2", None).await.unwrap();

        let context = manager.build_context(
            &source, Some("You are helpful")
        ).await.unwrap();
    ...
    async fn test_persistence_disabled() {
        ...
        manager.save_user_message(&source, "Hello"
        ).await.unwrap();
    ...
    async fn test_message_count() {
        ...
        let count = manager.message_count(
            &source
        ).await.unwrap();
        assert_eq!(count, 2);
    }
    ...
    async fn test_clear_session() {
        ...
        let deleted = manager.clear_session(
            &source
        ).await.unwrap();
        assert_eq!(deleted, 2);

        let session_id = SessionManager::session_id(
            &source);
    ...
    async fn test_session_isolation() {
        ...
        let ctx1 = manager.build_context(
            &source1, None
        ).await.unwrap();
        let ctx2 = manager.build_context(
            &source2, None
        ).await.unwrap();
