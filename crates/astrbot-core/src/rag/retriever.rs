...
impl Retriever {
    ...
    pub fn with_rerank(mut self, rerank: Arc<dyn RerankProvider>) -> Self {
    ...
    pub async fn delete_document(&self, doc_id: &str) -> Result<()> {
        self.store.delete(&self.collection, doc_id).await?;
        Ok(())
    }
}
...
mod tests {
    ...
    async fn test_retriever_index_and_query() {
        ...
        let splitter = TextSplitter::new(crate::rag::splitter::SplitStrategy::FixedSize {
            chunk_size: 20,
            overlap: 5,
        });
    ...
    #[tokio::test]
    async fn test_retriever_with_rerank() {
        let retriever = Retriever::new(
            Arc::new(MockEmbeddingProvider),
            Arc::new(MemoryVectorStore::new()),
            "test-col-rerank",
    ...
    #[tokio::test]
    async fn test_retriever_empty_document() {
        let retriever = Retriever::new(
            Arc::new(MockEmbeddingProvider),
            Arc::new(MemoryVectorStore::new()),
            "test-empty",
