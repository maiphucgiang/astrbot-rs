...
impl Provider for BaiduProvider {
    ...
    async fn chat(&self, messages: Vec<ChatMessage>, config: ChatConfig) -> Result<ChatResponse> { self.inner.chat(messages, config).await }
    async fn chat_stream(&self, messages: Vec<ChatMessage>, config: ChatConfig) -> Result<Box<dyn Stream<Item = Result<ChatStreamChunk>> + Send>> { self.inner.chat_stream(messages, config).await }
    async fn embedding(&self, texts: Vec<String>, model: Option<String>) -> Result<Vec<Vec<f32>>> { self.inner.embedding(texts, model).await }
...
mod tests {
    use super::*;
    use astrbot_core::provider::{ChatMessage, ChatConfig};

    #[tokio::test]
    async fn test_baidu_provider_creation() {
        let provider = BaiduProvider::new("baidu".to_string(), "test-key".to_string(), "ernie-4.0-turbo-8k".to_string());
