+++ b/crates/astrbot-provider/src/cohere.rs
@@ -0,0 +1,66 @@
+//! Cohere provider — OpenAI-compatible API
+//!
...
+        let base_url = "https://api.cohere.com/compatibility/v1".to_string();
+        Self {
...
+            "cohere-1".to_string(),
+            "sk-cohere-test".to_string(),
+            "command-r-plus".to_string(),
+        );
+        assert_eq!(p.id(), "cohere-1");
+        assert_eq!(p.name(), "cohere-1");
+    }
+}
