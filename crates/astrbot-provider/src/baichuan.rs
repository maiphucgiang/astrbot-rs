+++ b/crates/astrbot-provider/src/baichuan.rs
@@ -0,0 +1,66 @@
+//! Baichuan AI provider — OpenAI-compatible API
+//!
+//! Baichuan provides the Baichuan series models via an OpenAI-compatible API.
...
+        let base_url = "https://api.baichuan-ai.com/v1".to_string();
+        Self {
...
+
+    #[test]
+    fn test_baichuan_provider_creation() {
+        let p = BaichuanProvider::new(
+            "baichuan-1".to_string(),
+            "sk-baichuan-test".to_string(),
+            "Baichuan4".to_string(),
+        );
+        assert_eq!(p.id(), "baichuan-1");
+        assert_eq!(p.name(), "baichuan-1");
+    }
+}
